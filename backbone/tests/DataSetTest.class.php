...
class DatatSetTest extends TestSuite
{
	...
	public function testSimple()
	{
		...
		// set a simple value
		$this->caption("set a simple value");
		$set->set("foo", "one");
		$this->is(
			$this->isEqual($set->get("foo"), "one")
		);
		// change a simple value
		$this->caption("change a simple value");
		$set->set("foo", "two");
		$this->is(
			$this->isEqual($set->get("foo"), "two")
		);
		// unset the same value
		$this->caption("unset the same value");
		$set->set("foo", null);
		$this->is(
	...
	public function testNested()
	{
		...
		// set a nested value
		$this->caption("set a nested value");
		...
		$this->caption("change the nested value");
		...
		$this->caption("unset the nested value");
		$set->set("foo.homer", null);
		$this->is(
			$this->isNull($set->get("foo.homer"))
		);
		// test three levels
		$this->caption("test three levels");
